(() => {
  ...
  if (window.__monCarnetCoursesV254) return;
  window.__monCarnetCoursesV254 = true;
  ...
  const SAFE_BACKUP_DB_NAME = 'mon-carnet-cuisine-v1';
  const SAFE_BACKUP_DB_VERSION = 1;

  function integrationStorageBackup() {
    const saved = {};
    for (let index = 0; index < localStorage.length; index += 1) {
      const key = localStorage.key(index) || '';
      const allowed = key === STORAGE_KEY
        || key === PRODUCTS_STORAGE_KEY
        || /^claircourses/i.test(key);
      if (!allowed) continue;
  ...
  function openSafeBackupDb() {
    return new Promise((resolve, reject) => {
      if (!('indexedDB' in window)) {
        reject(new Error('IndexedDB indisponible'));
        return;
      }
      const request = indexedDB.open(SAFE_BACKUP_DB_NAME, SAFE_BACKUP_DB_VERSION);
  ...
  function replaceSafeBackupStore(database, storeName, records) {
    return new Promise((resolve, reject) => {
  ...
  function safeSettingsRecords(settings) {
  ...
  async function restoreSafeCompleteBackup(file) {
    if (!file) return;
    ...
    try {
      ...
      const database = await openSafeBackupDb();
      await replaceSafeBackupStore(database, 'recipes', payload.recipes);
      await replaceSafeBackupStore(database, 'plans', payload.plans);
      await replaceSafeBackupStore(
        database,
        'settings',
        safeSettingsRecords(payload.settings)
      );
      database.close();
  ...
  function armNativeCompleteExport() {
    ...
    JSON.stringify = function(value, replacer, space) {
      if (
        ...
        && value
        && typeof value === 'object'
        && value.app === 'Mon carnet de cuisine'
        ...
        return originalStringify.call(JSON, completePayload, replacer, space);
      }
  ...
  function installSafeCompleteBackup() {
    const exportButton = $('#exportBtn');
    if (exportButton && exportButton.dataset.mcSafeBackup !== '1') {
      exportButton.dataset.mcSafeBackup = '1';
      exportButton.textContent = 'Sauvegarder tout';
      exportButton.addEventListener('click', armNativeCompleteExport, true);

      const card = exportButton.closest('.setting-card');
      if (card && !card.querySelector('#mcSafeBackupNote')) {
        const note = document.createElement('p');
        note.id = 'mcSafeBackupNote';
    ...
    if (importInput && importInput.dataset.mcSafeBackup !== '1') {
      importInput.dataset.mcSafeBackup = '1';
      importInput.addEventListener('change', event => {
        const file = event.target.files?.[0];
        if (!file) return;
        event.preventDefault();
        event.stopImmediatePropagation();
        restoreSafeCompleteBackup(file).finally(() => {
          event.target.value = '';
        });
      }, true);
  ...
  function adaptExistingInterface() {
    document.title = 'Mon Carnet V2.5.4 TEST';
    setTextIfChanged($('.brand small'), 'VERSION · V2.5.4 TEST');
