(() => {
  ...
  if (window.__monCarnetCoursesV250) return;
  window.__monCarnetCoursesV250 = true;
  ...
  const FULL_BACKUP_DB_NAME = 'mon-carnet-cuisine-v1';
  const FULL_BACKUP_DB_VERSION = 1;
  ...
  function openFullBackupDb() {
    return new Promise((resolve, reject) => {
      if (!('indexedDB' in window)) {
        reject(new Error('Le stockage des recettes est indisponible'));
        return;
      }
      const request = indexedDB.open(FULL_BACKUP_DB_NAME, FULL_BACKUP_DB_VERSION);
  ...
  function readBackupStore(database, storeName) {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => reject(request.error || new Error(`Lecture impossible : ${storeName}`));
      request.onsuccess = () => resolve(Array.isArray(request.result) ? request.result : []);
    });
  }

  function replaceBackupStore(database, storeName, records) {
    return new Promise((resolve, reject) => {
      ...
      transaction.onerror = () => reject(transaction.error || new Error(`Restauration impossible : ${storeName}`));
      transaction.onabort = () => reject(transaction.error || new Error(`Restauration interrompue : ${storeName}`));
    });
  }

  function settingsRecordsToObject(records) {
    const result = {};
  ...
  function settingsObjectToRecords(settings) {
    if (Array.isArray(settings)) {
      return settings.filter(record => record && typeof record.key === 'string');
    }
    if (!settings || typeof settings !== 'object') return [];
    return Object.entries(settings).map(([key, value]) => ({ key, value }));
  }

  function integrationStorageBackup() {
    const saved = {};
    for (let index = 0; index < localStorage.length; index += 1) {
      const key = localStorage.key(index) || '';
      const isIntegrationKey = key === STORAGE_KEY || key === PRODUCTS_STORAGE_KEY;
      const isClairCoursesKey = /^claircourses/i.test(key);
      if (!isIntegrationKey && !isClairCoursesKey) continue;
  ...
  function restoreIntegrationStorage(saved) {
    if (!saved || typeof saved !== 'object') return;
    const keysToRemove = [];
    for (let index = 0; index < localStorage.length; index += 1) {
      const key = localStorage.key(index) || '';
      if (key === STORAGE_KEY || key === PRODUCTS_STORAGE_KEY || /^claircourses/i.test(key)) {
    ...
    Object.entries(saved).forEach(([key, value]) => {
      ...
      const allowed = key === STORAGE_KEY || key === PRODUCTS_STORAGE_KEY || /^claircourses/i.test(key);
      if (allowed) localStorage.setItem(key, value);
    });
  }


  function ensureFullBackupSaveDialog() {
    if ($('#mcFullBackupSaveLayer')) return;

    const style = document.createElement('style');
    style.id = 'mcFullBackupSaveStyle';
    style.textContent = `
      #mcFullBackupSaveLayer {
        position: fixed;
        inset: 0;
        z-index: 100000;
        display: none;
        place-items: center;
        padding: 22px;
        background: rgba(23,33,30,.62);
      }
      #mcFullBackupSaveLayer.open { display: grid; }
      #mcFullBackupSaveCard {
        width: min(430px, 100%);
        padding: 24px;
        border-radius: 24px;
        background: #fffdf8;
      ...
      #mcFullBackupSaveCard h2 {
        margin: 0 0 10px;
        font-family: Georgia, "Times New Roman", serif;
        font-weight: 500;
      }
      #mcFullBackupSaveCard p {
        margin: 0 0 18px;
        color: #68736f;
        line-height: 1.45;
      }
      #mcFullBackupSaveFile {
        width: 100%;
        min-height: 52px;
        border: 0;
        border-radius: 15px;
    ...
    layer.id = 'mcFullBackupSaveLayer';
    layer.setAttribute('role', 'dialog');
    layer.setAttribute('aria-modal', 'true');
    layer.innerHTML = `
      <div id="mcFullBackupSaveCard">
        <h2>Sauvegarde prête</h2>
        <p>Appuie ci-dessous, puis choisis <strong>Enregistrer dans Fichiers</strong>.</p>
        <button id="mcFullBackupSaveFile" type="button">Enregistrer dans Fichiers</button>
        <button id="mcFullBackupSaveLater" type="button">Plus tard</button>
      </div>
    `;
    document.body.appendChild(layer);

    $('#mcFullBackupSaveLater')?.addEventListener('click', () => {
      layer.classList.remove('open');
    });

    $('#mcFullBackupSaveFile')?.addEventListener('click', async () => {
      const pending = window.__mcPendingFullBackup;
      if (!pending?.blob || !pending?.filename) return;

      const saveButton = $('#mcFullBackupSaveFile');
      const originalText = saveButton.textContent;
      saveButton.disabled = true;
      saveButton.textContent = 'Préparation…';
      ...
      try {
        ...
        if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
          await navigator.share({
            files: [file],
            title: 'Sauvegarde Mon Carnet'
          });
          layer.classList.remove('open');
          toast('Choix d’enregistrement ouvert');
          return;
        }
        ...
        if (!error || error.name !== 'AbortError') {
          window.alert('L’enregistrement n’a pas pu s’ouvrir.');
        }
      } finally {
  ...
  function showFullBackupSaveDialog(filename, blob) {
    ensureFullBackupSaveDialog();
    window.__mcPendingFullBackup = { filename, blob };
    $('#mcFullBackupSaveLayer')?.classList.add('open');
  }
  ...
  async function exportFullBackup(event) {
    ...
    const originalText = button?.textContent || 'Exporter';
    if (button) {
    ...
    try {
      const database = await openFullBackupDb();
      const [recipes, plans, settingsRecords] = await Promise.all([
        readBackupStore(database, 'recipes'),
        readBackupStore(database, 'plans'),
        readBackupStore(database, 'settings')
      ]);

      const exportedAt = new Date().toISOString();
      const settings = settingsRecordsToObject(settingsRecords);
      settings.lastBackupAt = exportedAt;

      const payload = {
        app: 'Mon carnet de cuisine',
        format: 'mon-carnet-complet',
        version: '2.5.0',
        exportedAt,
        recipes,
        plans,
        ...
        integration: {
          version: '2.5.0',
          localStorage: integrationStorageBackup()
        }
      };

      const filename = `mon-carnet-complet-${backupDateStamp()}.json`;
      const blob = new Blob(
        [JSON.stringify(payload, null, 2)],
        { type: 'application/json' }
      );

      showFullBackupSaveDialog(filename, blob);

      await replaceBackupStore(
        database,
        'settings',
        settingsObjectToRecords(settings)
      );
      database.close();

      const status = $('#backupStatus');
      if (status) status.textContent = `Dernière sauvegarde complète : ${new Date(exportedAt).toLocaleString('fr-FR')}`;
      toast('Sauvegarde prête à enregistrer');
    } catch (error) {
      console.error(error);
      toast('La sauvegarde n’a pas pu être créée');
    } finally {
      if (button) {
  ...
  async function importFullBackup(event) {
    ...
    try {
      ...
      if (!payload || !Array.isArray(payload.recipes) || !Array.isArray(payload.plans)) {
        throw new Error('Fichier de sauvegarde non reconnu');
      }

      const confirmation = window.confirm(
        `Restaurer cette sauvegarde remplacera les recettes, le planning, les réglages, Mes produits et Mes courses actuels.\n\nContinuer ?`
      );
      if (!confirmation) return;

      const database = await openFullBackupDb();
      await replaceBackupStore(database, 'recipes', payload.recipes);
      await replaceBackupStore(database, 'plans', payload.plans);
      await replaceBackupStore(
        database,
        'settings',
        settingsObjectToRecords(payload.settings)
      );
      database.close();

      if (payload.integration?.localStorage) {
        restoreIntegrationStorage(payload.integration.localStorage);
      }

      window.alert('Sauvegarde restaurée. Mon Carnet va redémarrer.');
      window.location.reload();
    } catch (error) {
      console.error(error);
      window.alert('Ce fichier ne peut pas être restauré.');
    } finally {
      if (input) input.value = '';
    }
  }

  function installFullBackupBridge() {
    ensureFullBackupSaveDialog();
    const exportButton = $('#exportBtn');
    const importInput = $('#importInput');

    if (exportButton && exportButton.dataset.mcFullBackup !== '1') {
      exportButton.dataset.mcFullBackup = '1';
      exportButton.textContent = 'Sauvegarder tout';
      exportButton.addEventListener('click', exportFullBackup, true);

      const card = exportButton.closest('.setting-card');
      if (card && !card.querySelector('#mcFullBackupNote')) {
        const note = document.createElement('p');
        note.id = 'mcFullBackupNote';
        note.textContent = 'Inclut aussi Mes produits et Mes courses.';
    ...
    if (importInput && importInput.dataset.mcFullBackup !== '1') {
      importInput.dataset.mcFullBackup = '1';
      importInput.addEventListener('change', importFullBackup, true);
      const label = document.querySelector('label[for="importInput"]');
      if (label) label.textContent = 'Restaurer tout';
    }
  }

  function adaptExistingInterface() {
    document.title = 'Mon Carnet V2.5.0 TEST';
    setTextIfChanged($('.brand small'), 'VERSION · V2.5.0 TEST');
