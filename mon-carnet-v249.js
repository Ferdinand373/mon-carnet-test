(() => {
  ...
  if (window.__monCarnetCoursesV249) return;
  window.__monCarnetCoursesV249 = true;
  ...
  async function exportFullBackup(event) {
    ...
    try {
      ...
      try {
        if (window.parent && typeof window.parent.__mcReceiveBackup === 'function') {
          window.parent.__mcReceiveBackup({ filename, blobUrl });
          handedOff = true;
        }
      } catch (_) {}
  ...
  function adaptExistingInterface() {
    document.title = 'Mon Carnet V2.4.9 TEST';
    setTextIfChanged($('.brand small'), 'VERSION · V2.4.9 TEST');
